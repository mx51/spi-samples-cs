    ...
    internal class MotelPos
    {
                                ...
                                case TransactionType.Preauth:
                                    {
                                        ...
                                        Console.WriteLine(!details.WasCustomerReceiptPrinted() ? details.GetCustomerReceipt().TrimEnd() : "# PRINTED FROM EFTPOS");
                                        break;
                                    }
                                case TransactionType.AccountVerify:
                                    {
                                        ...
                                        Console.WriteLine(!details.WasMerchantReceiptPrinted() ? details.GetMerchantReceipt().TrimEnd() : "# PRINTED FROM EFTPOS");
                                        break;
                                    }
                                default:
        ...
        private void PrintActions()
        {
            ...
            if (_spi.CurrentFlow == SpiFlow.Idle)
            {
                ...
                Console.WriteLine("# [preauth_complete:12345678:8000:100] - Complete Preauth with id 12345678 for $80.00 and $1.00 surcharge");
        ...
        private void PrintPairingStatus()
        {
            ...
            Console.WriteLine($"# SPI CONFIG: {_spiPreauth.Config}");
        ...
        private bool ProcessInput(string[] spInput)
        {
            ...
            switch (spInput[0].ToLower())
            {
                ...
                case "preauth_complete":
                    preauthId = spInput[1];
                    initRes = _spiPreauth.InitiateCompletionTx("prcomp-" + preauthId + "-" + DateTime.Now.ToString("dd-MM-yyyy-HH-mm-ss"), preauthId, int.Parse(spInput[2]), int.Parse(spInput[3]));
                    if (!initRes.Initiated)
                    {
                        Console.WriteLine($"# Could not initiate preauth request: {initRes.Message}. Please Retry.");
                    }
                ...
                case "print_merchant_copy":
                    _spiPreauth.Config.PrintMerchantCopy = spInput[1].ToLower() == "true";
                    Console.Write("> ");
                    break;
                case "rcpt_from_eftpos":
                    _spiPreauth.Config.PromptForCustomerCopyOnEftpos = spInput[1].ToLower() == "true";
                    Console.Write("> ");
                    break;
                case "sig_flow_from_eftpos":
                    _spiPreauth.Config.SignatureFlowOnEftpos = spInput[1].ToLower() == "true";
                    Console.Write("> ");
                    break;
